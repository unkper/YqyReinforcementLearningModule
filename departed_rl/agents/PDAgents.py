...
from departed_rl.utils.networks.pd_network import Critic, Actor, SimpleCritic
from departed_rl.utils.updates import soft_update, hard_update
from departed_rl.utils.classes import SaveNetworkMixin, Noise
from departed_rl.utils.functions import back_specified_dimension, onehot_from_int
...
class DDPGAgent(Agent, SaveNetworkMixin):
    ...
    def __init__(self, env: Env = None,
        ...
        self.actor = Actor(self.state_dim,
                           self.action_dim,
                           self.discrete).to(self.device) if actor_network == None else actor_network(self.state_dim,
                                                                                                      self.action_dim)
        self.target_actor = Actor(self.state_dim,
                                  self.action_dim,
                                  self.discrete).to(self.device) if actor_network == None else actor_network(
            self.state_dim, self.action_dim)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(),
                                                self.learning_rate)
        # Critic根据(s,a)输出一个Q值
        # Actor根据s输出一个a值(连续值),通过加入噪声达到随机的机制
        self.critic = SimpleCritic(self.state_dim, self.action_dim).to(
            self.device) if critic_network is None else critic_network(self.state_dim, self.action_dim)
        self.target_critic = SimpleCritic(self.state_dim, self.action_dim).to(
            self.device) if critic_network is None else critic_network(self.state_dim, self.action_dim)
    ...
    def _learn_from_memory(self):
        ...
        # 软更新参数
        if self.total_steps_in_train % 100 == 0:
    ...
    def learning_method(self,
        ...
        if self.total_steps_in_train % 500 == 0:

...
class PedEnvWrapper(BaseWrapper):
    def __init__(self, env, joint_count=False, use_adv_network=False, use_concat_obs=False, use_key_point=False):
        ...
        self._prv_obs = None
        self.wrapped = not isinstance(self.env, PedsMoveEnv)

    def reset(self):
        obs_dict = self.env.reset()
        iters = self.env.possible_agents if not self.wrapped else self.env.control_list
        _obs = []
        for agent in iters:
        ...
        if self.joint_count:
            visit_inds = tuple(sum([[int(a.x), int(a.y)] for a in iters], []))
    ...
    def step(self, actions):
        iters = self.env.possible_agents if not self.wrapped else self.env.control_list
        acts = {}
        for i, agent in enumerate(self.env.agents if not self.wrapped else self.env.control_list):
        ...
        _info = {}
        for agent in iters:
            if not self.wrapped or next_o is not None:
        ...
        self._prv_obs = _obs
        #time.sleep(0.05)
        # 注意该接口返回的rew的是所有agent奖励的平均值，all代表所有智能体都为done后

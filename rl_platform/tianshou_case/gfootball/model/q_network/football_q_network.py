...
class FootballNaiveQ(nn.Module):
    ...
    def __init__(
        ...
        self.scalar_encoder = ScalarEncoder(cfg=scalar_encoder_arch) # 全连接层进行encode
...
class ScalarEncoder(nn.Module):
    # 使用全连接层对输入的状态进行编码，使用了get_attr和set_attr方法
    def __init__(self, cfg: dict) -> None:
        ...
        for k, arch in cfg.items():
            ...
            encoder = fc_block(arch['input_dim'], arch['output_dim'], activation=self.act) #创建了一个全连接用于处理输入
...
class PlayerEncoder(nn.Module):
    # 设置了Transformer作为Player的Encoder
    def __init__(
            self,
            cfg: dict,
    ) -> None:

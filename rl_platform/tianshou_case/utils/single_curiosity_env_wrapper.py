...
class CuriosityEnvWrapper:
    ...
    def _postprocess_observation(self, observation, reward=None):
        if not self._should_postprocess_observation(observation[0].shape):
            return observation

        if self._append_ec_reward_as_channel:
            if reward is not None:
                return resize_observation(observation[0], self._target_image_shape, reward)
            else:
                # When environment is reset there are no rewards, so we explicitly pass
                # 0 in this case.
                return resize_observation(observation[0], self._target_image_shape, 0)
        else:
            return resize_observation(observation[0], self._target_image_shape, None)

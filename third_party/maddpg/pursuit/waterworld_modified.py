...
class EzPickle(object):
    ...
    furcolor and tailkind i nto the constructor. However, philosophers are still not sure
...
class Archea(Agent):
    ...
    def sensed(self, objx_N_2, same=False):
        ...
        sensorvals_K_N[(sensorvals_K_N < 0) | (sensorvals_K_N > self._sensor_range) | ((
                                                                                               relpos_obj_N_2 ** 2).sum(
            axis=1)[None, :] - sensorvals_K_N ** 2 > self._radius ** 2)] = np.inf
        if same:
...
class MAWaterWorld_mod(AbstractMAEnv, EzPickle):
    ...
    def _respawn(self, objx_2, radius):
        while ssd.cdist(objx_2[None, :], self.obstaclesx_No_2) <= radius * 2 + self.obstacle_radius:
            objx_2 = self.np_random.random(2)
        return objx_2
    ...
    def reset(self):
        ...
        if self.obstacle_loc is None:
            self.obstaclesx_No_2 = self.np_random.random(self.n_obstacles, 2)
        else:
        ...
        for pursuer in self._pursuers:
            pursuer.set_position(self.np_random.random(2))
        ...
        for evader in self._evaders:
            evader.set_position(self.np_random.random(2))
            evader.set_position(self._respawn(evader.position, evader._radius))
            evader.set_velocity((self.np_random.random(2) - 0.5) * self.ev_speed)  # TODO policies

        # Initialize poisons
        for poison in self._poisons:
            poison.set_position(self.np_random.random(2))
            poison.set_position(self._respawn(poison.position, poison._radius))
            poison.set_velocity((self.np_random.random(2) - 0.5) * self.ev_speed)
    ...
    def step(self, action_Np2):
        ...
        if ev_caught.size:
            for evcaught in ev_caught:
                self._evaders[evcaught].set_position(self.np_random.random(2))
                ...
                self._evaders[evcaught].set_velocity(
                    (self.np_random.random(2, ) - 0.5) * self.ev_speed)

        if po_caught.size:
            for pocaught in po_caught:
                self._poisons[pocaught].set_position(self.np_random.random(2))
                ...
                self._poisons[pocaught].set_velocity(
                    (self.np_random.random(2, ) - 0.5) * self.poison_speed)
...
if __name__ == '__main__':
    env = MAWaterWorld_mod(1, 2, obs_loc=None)
    obs = env.reset()
    while True:
        obs, rew, done, info = env.step(env.np_random.random(2) - .5)
        if rew.sum() > 0:
            print(rew)
        env.render()

...
class ReplayBuffer(object):
    ...
    def __init__(self, max_steps, num_agents, state_space, observation_space, action_space):
        ...
        for i in range(self.num_agents):
            if type(observation_space) is spaces.Tuple:
                self.mult_obs = True
                agent_obs_buff = [np.zeros((max_steps, *space.shape), dtype=space.dtype) for space in
                                  observation_space.spaces]
                self.obs_buffs.append(agent_obs_buff)
                agent_next_obs_buff = [np.zeros((max_steps, *space.shape), dtype=space.dtype) for space in
                                       observation_space.spaces]
    ...
    def sample(self, N, to_gpu=False, norm_rews=False, state_inds=False, recent=None):
        ...
        if self.mult_obs:
            ret_obs = [tuple(cast(self.obs_buffs[ai][oi][inds]) for oi in range(len(self.obs_buffs[ai]))) for ai in
                       range(self.num_agents)]
            ret_next_obs = [tuple(cast(self.next_obs_buffs[ai][oi][inds]) for oi in range(len(self.next_obs_buffs[ai])))
                            for ai in range(self.num_agents)]
        else:
